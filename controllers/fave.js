...
async function getFavePage(req, res) {
    try {
        ...
        let faveItems = await Promise.all(faveItemsInfo.map(async (itemInfo) => {
            console.log('Fetching product details for item:', itemInfo._id);
            const item = await productsService.getProductById(itemInfo._id);
            if (item) {
                item.size = itemInfo.size;
                sum += item.price || 0;
                return item;
            } else {
                console.log('Product not found for id:', itemInfo._id);
            }
            return null;
        }));
...
async function getFaveItems(req, res) {
    try {
        ...
        const faveItems = await clientsService.getFaveIetemsFromDB(username);
